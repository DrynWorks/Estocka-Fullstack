...
const navigation = [
    ...
    { name: 'Produtos', href: '/products', icon: Package, resource: 'products' },
    { name: 'Movimentações', href: '/movements', icon: ArrowLeftRight, resource: 'movements' },
    { name: 'Relatórios', href: '/reports', icon: BarChart3, resource: 'reports' },
    { name: 'Usuários', href: '/users', icon: Users, resource: 'users' },
    { name: 'Auditoria', href: '/audit', icon: FileText, resource: 'audit' },
];
            ...
            >
                <div className="h-full px-5 py-6 overflow-y-auto">
                    ...
                    <ul className="space-y-1.5">
                        {navigation.map((item) => {
                            // Special case for Dashboard: always show if authenticated (or maybe check if user exists)
                            if (item.resource !== 'dashboard' && !canView(item.resource)) return null;
